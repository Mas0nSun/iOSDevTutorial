...
struct AlbumWidgetView: View {
    ...
    private var playMusicIntent: PlayMusicIntent {
        let intent = PlayMusicIntent()
        intent.albumId = albumId
        intent.songId = songId
        intent.musicTitle = title
        ...
        VStack(spacing: 4) {
            ...
            if isWidget {
                Button(intent: playMusicIntent) {
                    vinylRecordAndCenterAlbumCoverView(size: size)
                }
    ...
    // 唱片效果层 + 中心封面图片
    private func vinylRecordAndCenterAlbumCoverView(size: CGFloat) -> some View {
        // App 中，显示正常的唱片
        ZStack {
            // 唱片效果层
