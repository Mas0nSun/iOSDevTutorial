...
extension MusicKitManager {
    ...
    func play(widget: AlbumWidgetData) async {
        ...
        if !isAuthorized {
            ...
            guard isAuthorized else {
                self.errorMessage = "未获得 Apple Music 权限，无法播放"
                return
            }
        }
        do {
            if let songId = widget.songId, !songId.isEmpty {
                try await playSong(id: songId)
            } else if let albumId = widget.albumId, !albumId.isEmpty {
                try await playAlbum(id: albumId)
            } else {
                self.errorMessage = "无效的音乐资源 ID"
            }
        } catch {
